...
public class c5_CreatingSequence {
    ...
    public void repeat() {
        ...
        Callable<Integer> incrementAndGetCallable = counter::incrementAndGet;

        Flux<Integer> repeated = Mono.fromCallable(incrementAndGetCallable).repeat(9);
    ...
    public void generate_programmatically() {
        ...
        Flux<Integer> createFlux = Flux.create(sink -> {
            IntStream.iterate(0, i -> i + 1)
                     .limit(6)
                     .mapToObj(it -> es.submit(() -> sink.next(it)))
                     .forEach(it -> ExceptionalSupplier.safe(it::get));
    ...
    interface ExceptionalSupplier<R> {
        R accept() throws Exception;

        static <R> R safe(ExceptionalSupplier<R> supplier) {
            try {
