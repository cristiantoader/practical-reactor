...
public class c6_CombiningPublishers extends CombiningPublishersBase {
    ...
    public void i_am_rubber_you_are_glue() {
        Flux<Integer> serviceOneFlux = numberService1();
        Flux<Integer> serviceTwoFlux = numberService2();

        Flux<Integer> numbers = Flux.concat(serviceOneFlux, serviceTwoFlux);
    ...
    public void mail_box_switcher() {
        Flux<Message> primaryMailFlux = mailBoxPrimary();
        Flux<Message> secondaryMailFlux = mailBoxSecondary();

        Flux<Message> myMail = primaryMailFlux.switchOnFirst((signal, flux) -> {
            if (signal.hasValue() && signal.get().metaData.equals("spam")) {
                return secondaryMailFlux;
            }
    ...
    public void acid_durability() {
        //todo: feel free to change code as you need
        var committedTasksIds = tasksToExecute()
                .concatMap(responseMono -> responseMono.flatMap(taskId -> commitTask(taskId).thenReturn(taskId)));
    ...
    public void car_factory() {
        ...
        Flux<Car> producedCars = Flux.zip(chassisFlux, engineFlux)
    ...
    public Mono<String> chooseSource() {
        // executes lazy
        return Mono.defer(() -> switch (sourceRef.get()) {
            case "A" -> sourceA();
            case "B" -> sourceB();
            default -> Mono.empty();
        });
    ...
    public void cleanup() {
        ...
        Flux<String> stream = Flux.usingWhen(StreamingConnection.startStreaming(),
                Function.identity(),
                it -> StreamingConnection.closeConnection());
