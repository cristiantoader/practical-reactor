...
public class c7_ErrorHandling extends ErrorHandlingBase {
    ...
    public void have_a_backup() {
        Flux<String> messageFlux = messageNode();
        Flux<String> backupMessageFlux = backupMessageNode();

        Flux<String> messages = messageFlux.onErrorResume(e -> backupMessageFlux);
    ...
    public void error_reporter() {
        Flux<String> messages = messageNode()
                .onErrorResume(err -> errorReportService(err).then(Mono.error(err)));
